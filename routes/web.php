...
Route::middleware([
    ...
    Route::middleware(['onboarding'])->group(function () {
        ...
        Route::post('/dashboard/professionals/bulk', [ProfessionalController::class, 'bulkStore'])
            ->name('dashboard.professionals.bulk-store');
        ...
        Route::post('/dashboard/services/bulk', [ServiceController::class, 'bulkStore'])
            ->name('dashboard.services.bulk-store');
    ...
    Route::get('/onboarding/step/{step}', [OnboardingController::class, 'step'])
        ->where('step', '[1-3]')
...
Route::middleware(['public.booking'])->group(function () {
    Route::get('/booking/{slug}', [BookingController::class, 'index'])
        ->name('booking.index');
    Route::get('/booking/{slug}/services', [BookingController::class, 'services'])
        ->name('booking.services');
    Route::get('/booking/{slug}/professionals', [BookingController::class, 'professionals'])
        ->name('booking.professionals');
    Route::get('/booking/{slug}/professional', [BookingController::class, 'professional'])
        ->name('booking.professional');
    Route::get('/booking/{slug}/slots', [BookingController::class, 'slots'])
        ->name('booking.slots');
    Route::get('/booking/{slug}/confirm', [BookingController::class, 'confirm'])
        ->name('booking.confirm');
    Route::get('/booking/{slug}/success', [BookingController::class, 'success'])
        ->name('booking.success');
    Route::post('/booking/{slug}/appointments', [BookingController::class, 'store'])
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified', 'tenant'])->group(function () {
    Route::post('/booking/{slug}/token', [BookingController::class, 'generateToken'])
        ->name('booking.token.generate');
});
